-- Bootstrap all profiles inside SECURITY DEFINER RPC (avoids RLS on pets insert)
...
begin
  ...
  for profile_rec in select id from profiles loop
    ...
    case species
      when 'mamono' then element := (array['earth', 'nature'])[1 + floor(random() * 2)::int];
      when 'avian' then element := (array['wind', 'nature'])[1 + floor(random() * 2)::int];
      when 'aquatic' then element := (array['water', 'nature'])[1 + floor(random() * 2)::int];
