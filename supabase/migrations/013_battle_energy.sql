-- Ultimate energy (0–100%) replaces one-shot ultimate flag

alter table battle_sessions
  add column if not exists challenger_energy int not null default 0,
  add column if not exists defender_energy int not null default 0;

alter table battle_sessions
  add constraint battle_sessions_challenger_energy_chk check (challenger_energy between 0 and 100),
  add constraint battle_sessions_defender_energy_chk check (defender_energy between 0 and 100);
...
begin
  ...
  if p_action = 'defend' then
    ...
    if is_challenger then
      update battle_sessions
      set challenger_defending = true, challenger_energy = new_energy, turn_user_id = sess.defender_user_id
      where id = p_session_id returning * into sess;
    else
      update battle_sessions
      set defender_defending = true, defender_energy = new_energy, turn_user_id = sess.challenger_user_id
      where id = p_session_id returning * into sess;
    end if;

    msg := actor_name || ' ตั้งท่าป้องกัน (+' || energy_gain || '% พลัง)';
  ...
  if effective_action = 'ultimate' then
    msg := actor_name || ' ใช้ท่าไม้ตาย ' || opp_name || ' -' || dmg || ' HP';
  else
    msg := actor_name || ' โจมตี ' || opp_name || ' -' || dmg || ' HP (+' || energy_gain || '% พลัง)';
  end if;
